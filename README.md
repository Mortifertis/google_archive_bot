...
- сохраняет текст, фотографии и фотоальбомы;
- сохраняет видео и animation/GIF в пределах лимита Telegram Cloud Bot API;
- поддерживает смешанные фото/видео альбомы с сохранением порядка;
...
Видео и GIF сохраняются приватными обычными файлами в общей служебной папке
`Telegram Archive/Media`. Google Doc содержит thumbnail от Telegram и
кликабельную ссылку на бинарный файл. Фотографии по-прежнему встраиваются
непосредственно в DOCX и не создаются на Drive отдельными файлами.

Telegram Cloud Bot API позволяет боту скачать файл размером не более 20 MB.
Для более крупного видео или animation документ всё равно создаётся с
metadata, текстом, доступным thumbnail и пояснением о лимите, но бинарный
файл в папку `Media` не загружается.
...
6. Перешлите mixed photo/video album: должен появиться один документ с фото и
   видео в исходном порядке, а видео — в папке `Media`.
7. Перешлите video крупнее 20 MB: документ должен сохраниться с уведомлением
   о лимите, без нового бинарного файла на Drive.
...
Не поддерживаются audio, voice, video note, stickers, paid media и
произвольные documents. Группа с любым неподдерживаемым элементом целиком
отклоняется. Telegram может не разрешить скачать защищённый контент.
Deduplication возможно только при наличии `source_chat_id` и
`source_message_id` в Telegram metadata.
